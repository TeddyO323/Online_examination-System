...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_SESSION['reg_no'])) {
        ...
        if (isset($_GET['exam_id']) && !empty($_GET['exam_id'])) {
            ...
            // Fetch the time_limit for the exam from exam_tbl
            $sqlTimeLimit = "SELECT exam_time_limit FROM exam_tbl WHERE ex_id = '$exam_id'";
            $resultTimeLimit = $conn->query($sqlTimeLimit);

            if ($resultTimeLimit && $resultTimeLimit->num_rows > 0) {
                $rowTimeLimit = $resultTimeLimit->fetch_assoc();
                $time_limit = $rowTimeLimit['exam_time_limit'];

                // Insert data into the exam_attempt table with the time_limit
                $exam_attempt_stmt = $conn->prepare("INSERT INTO exam_attempt (reg_no, exam_id, time_limit) VALUES (?, ?, ?)");

                if ($exam_attempt_stmt) {
                    $exam_attempt_stmt->bind_param("sid", $reg_no, $exam_id, $time_limit);
